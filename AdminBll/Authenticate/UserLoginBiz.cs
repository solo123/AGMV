...
{
    public class UserLoginBiz : AdminBll
    {
        public D_LoginUserInfo loginUserInfo;
...
";
            ...
            if (o != null)
            {
                ...
                loginUserInfo = biz.GetLoginUserInfo((int)o);

                ActionAuthorization auth = new ActionAuthorization();
                if (auth.CheckAuthorization(loginUserInfo,"LoginAfterWorkTime"))
                    r = true;
                else
                {
                    DateTime now = DateTime.Now;
                    r = (now.Hour >= 7 && now.Hour <= 20);
                }
            }

            if (r)
            {
                EventLog.OpLog(loginUserInfo, "Success Login", 1, loginUserInfo.userId);
            }
            return r;
        }
        #endregion
